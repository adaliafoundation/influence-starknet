...
mod tests {
    ...
    use super::{Crew, ICrewDispatcher, ICrewDispatcherTrait};
    ...
    fn test_constructor() {
        ...
        let mut state = Crew::contract_state_for_testing();
        Crew::constructor(ref state, 'Influence Crews', 'INFCRW', caller);
        let res = Crew::has_grant(@state, caller, Crew::roles::ADMIN);
    ...
    fn test_grants() {
        ...
        Crew::add_grant(ref state, caller, Crew::roles::MINTER);
        let res = Crew::has_grant(@state, caller, Crew::roles::MINTER);
        assert(res, 'caller should be minter');

        Crew::revoke_grant(ref state, caller, Crew::roles::MINTER);
        let res = Crew::has_grant(@state, caller, Crew::roles::MINTER);
    ...
    fn test_mint_without_grant() {
        ...
        Crew::mint_with_auto_id(ref state, caller);
    }
    ...
    fn test_mint() {
        ...
        let token_id = Crew::mint_with_auto_id(ref state, caller);
        let res = Crew::ownerOf(@state, token_id);
    ...
    fn test_transfer_with_restriction() {
        ...
        let normal_token_id = Crew::mint_with_auto_id(ref state, caller);
        assert(!Crew::is_restricted(@state, normal_token_id), 'normal token restricted');

        let restricted_token_id = Crew::mint_with_auto_id(ref state, caller);
        Crew::transfer_with_restriction(ref state, caller, receiver, restricted_token_id, 200, restriction_authority);
        assert(Crew::is_restricted(@state, restricted_token_id), 'not restricted');
        assert(Crew::ownerOf(@state, restricted_token_id) == receiver, 'wrong owner');
        let restriction = Crew::restriction(@state, restricted_token_id);
        ...
        assert(!Crew::is_restricted(@state, restricted_token_id), 'expired restriction active');
        let restriction = Crew::restriction(@state, restricted_token_id);
    ...
    fn test_transfer() {
        ...
        let token_id = Crew::mint_with_auto_id(ref state, caller);
        let receiver = starknet::contract_address_const::<'RECEIVER'>();
        Crew::transfer_from(ref state, caller, receiver, token_id);
        let res = Crew::ownerOf(@state, token_id);
    ...
    fn test_transfer_restricted() {
        ...
        let token_id = Crew::mint_with_auto_id(ref state, caller);
        let receiver = starknet::contract_address_const::<'RECEIVER'>();
        Crew::transfer_with_restriction(ref state, caller, receiver, token_id, 200, caller);
        starknet::testing::set_caller_address(receiver);
        Crew::transfer_from(ref state, receiver, caller, token_id);
    }
    ...
    fn test_recall_restricted() {
        ...
        let token_id = Crew::mint_with_auto_id(ref state, caller);
        Crew::transfer_with_restriction(ref state, caller, receiver, token_id, 200, restriction_authority);

        starknet::testing::set_caller_address(restriction_authority);
        Crew::recall_restricted(ref state, token_id);
        assert(Crew::ownerOf(@state, token_id) == restriction_authority, 'not recalled');
        assert(!Crew::is_restricted(@state, token_id), 'restriction not cleared');

        Crew::transfer_from(ref state, restriction_authority, receiver, token_id);
        assert(Crew::ownerOf(@state, token_id) == receiver, 'transfer failed');
    }
    ...
    fn test_transfer_wrong_sender() {
        ...
        let token_id = Crew::mint_with_auto_id(ref state, caller);
        let receiver = starknet::contract_address_const::<'RECIEVER'>();
        let wrong_sender = starknet::contract_address_const::<'WRONG_SENDER'>();
        Crew::transfer_from(ref state, wrong_sender, receiver, token_id);
    }
    ...
    fn test_transfer_invalid_receiver() {
        ...
        Crew::transfer_from(ref state, caller, invalid_receiver, token_id);
    }
    ...
    fn test_approve_other_sender() {
        ...
        let token_id = Crew::mint_with_auto_id(ref state, caller);
        Crew::approve(ref state, other_sender, token_id);
        let res = Crew::getApproved(@state, token_id);
    ...
    fn test_send_with_approval() {
        ...
        Crew::transfer_from(ref state, caller, other_sender, token_id);
        let res = Crew::ownerOf(@state, token_id);
    ...
    fn test_token_uri() {
        ...
        let base_uri = array!['https://api.influenceth.io/', 'metadata/crews/'];
        Crew::set_base_uri(ref state, base_uri);

        let token_id = Crew::mint_with_auto_id(ref state, caller);
        let res = Crew::token_uri(@state, token_id);
        assert(*res.at(0) == 'https://api.influenceth.io/', 'wrong base uri');
        assert(*res.at(1) == 'metadata/crews/', 'wrong base uri');
    ...
    fn test_bridge_from_l1() {
        ...
        let mut state = Crew::contract_state_for_testing();
        Crew::constructor(ref state, 'Influence Crew', 'INFCRW', caller);
        Crew::add_grant(ref state, caller, Crew::roles::MINTER);

        Crew::set_l1_bridge_address(ref state, 'L1_BRIDGE'.try_into().unwrap());
        ...
        let owner = Crew::ownerOf(@state, 42);
    ...
    fn test_erc165_response() {
        ...
        let mut res = Crew::supports_interface(@state, 0x01ffc9a7);
        assert(res, 'should support erc165');

        res = Crew::supports_interface(@state, 0x80ac58cd);
        assert(res, 'should support erc721');

        res = Crew::supports_interface(@state, 0x5b5e139f);
    ...
    fn test_set_sell_order() {
        ...
        let price = 1000_u128;
        Crew::mint_with_auto_id(ref state, caller);
        Crew::set_sell_order(ref state, token_id, price);
        assert(Crew::get_sell_order(@state, token_id) == price, 'wrong price');

        Crew::set_sell_order(ref state, token_id, 0);
        assert(Crew::get_sell_order(@state, token_id) == 0, 'wrong price');
    }

    #[test]
    #[available_gas(10000000)]
    fn test_fill_sell_order() {
        let ast_class_hash: ClassHash = Crew::TEST_CLASS_HASH.try_into().unwrap();
        let mut constructor_data: Array<felt252> = Default::default();
        constructor_data.append('Influence Crews');
        constructor_data.append('INFCRW');
        constructor_data.append('ADMIN');
        let (crew_address, _) = deploy_syscall(ast_class_hash, 0, constructor_data.span(), false).unwrap();
        let crew = ICrewDispatcher { contract_address: crew_address };
        ...
        crew.set_sway_address(sway_address);
        crew.add_grant(caller, Crew::roles::MINTER);

        let token_id = 1_u256;
        let price = 1000_u128;
        crew.mint_with_auto_id(caller);
        crew.set_sell_order(token_id, price);
        ...
        crew.fill_sell_order(token_id);
        assert(crew.get_sell_order(token_id) == 0, 'wrong price');
        assert(crew.owner_of(token_id) == buyer, 'wrong owner');
    }
}

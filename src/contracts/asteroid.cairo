...
#[starknet::interface]
trait IAsteroid<TContractState> {
...
mod Asteroid {
    ...
    const TOTAL_ASTEROIDS: u256 = 250000;
    ...
    fn mint_with_id(ref self: ContractState, to: ContractAddress, token_id: u256) {
        assert(token_id <= TOTAL_ASTEROIDS, 'Asteroid: invalid token ID');
    ...
    fn _mint(ref self: ContractState, to: ContractAddress, token_id: u256) -> u256 {
...
mod tests {
    ...
    use super::{Asteroid, IAsteroidDispatcher, IAsteroidDispatcherTrait};
    ...
    fn test_constructor() {
        ...
        let mut state = Asteroid::contract_state_for_testing();
        Asteroid::constructor(ref state, 'Influence Asteroids', 'INFAST', caller);
        let res = Asteroid::has_grant(@state, caller, Asteroid::roles::ADMIN);
    ...
    fn test_grants() {
        ...
        Asteroid::add_grant(ref state, caller, Asteroid::roles::MINTER);
        let res = Asteroid::has_grant(@state, caller, Asteroid::roles::MINTER);
        assert(res, 'caller should be minter');

        Asteroid::revoke_grant(ref state, caller, Asteroid::roles::MINTER);
        let res = Asteroid::has_grant(@state, caller, Asteroid::roles::MINTER);
    ...
    fn test_mint_without_grant() {
        ...
        Asteroid::mint_with_id(ref state, caller, 42_u256);
    }
    ...
    fn test_mint() {
        ...
        Asteroid::mint_with_id(ref state, caller, token_id);
        let res = Asteroid::ownerOf(@state, token_id);
    ...
    fn test_transfer() {
        ...
        Asteroid::transfer_from(ref state, caller, receiver, token_id);
        let res = Asteroid::ownerOf(@state, token_id);
    ...
    fn test_transfer_wrong_sender() {
        ...
        let receiver = starknet::contract_address_const::<'RECEIVER'>();
        let wrong_sender = starknet::contract_address_const::<'WRONG_SENDER'>();
        Asteroid::transfer_from(ref state, wrong_sender, receiver, token_id);
    }
    ...
    fn test_transfer_invalid_receiver() {
        ...
        Asteroid::transfer_from(ref state, caller, invalid_receiver, token_id);
    }
    ...
    fn test_approve_other_sender() {
        ...
        let token_id = 42_u256;
        Asteroid::mint_with_id(ref state, caller, token_id);
        Asteroid::approve(ref state, other_sender, token_id);
        let res = Asteroid::getApproved(@state, token_id);
    ...
    fn test_send_with_approval() {
        ...
        Asteroid::transfer_from(ref state, caller, other_sender, token_id);
        let res = Asteroid::ownerOf(@state, token_id);
    ...
    fn test_token_uri() {
        ...
        let base_uri = array!['https://api.influenceth.io/', 'metadata/asteroids/'];
        Asteroid::set_base_uri(ref state, base_uri);

        let token_id = 42_u256;
        Asteroid::mint_with_id(ref state, caller, token_id);
        let res = Asteroid::token_uri(@state, token_id);
        assert(*res.at(0) == 'https://api.influenceth.io/', 'wrong base uri');
        assert(*res.at(1) == 'metadata/asteroids/', 'wrong base uri');
    ...
    fn test_bridge_from_l1() {
        ...
        Asteroid::set_l1_bridge_address(ref state, 'L1_BRIDGE'.try_into().unwrap());
        let token_ids = array![42];
        Asteroid::bridge_from_l1(ref state, 'L1_BRIDGE', starknet::contract_address_const::<'PLAYER'>(), token_ids);

        let owner = Asteroid::ownerOf(@state, 42);
    ...
    fn test_erc165_response() {
        ...
        let mut res = Asteroid::supports_interface(@state, 0x01ffc9a7);
        assert(res, 'should support erc165');

        res = Asteroid::supports_interface(@state, 0x80ac58cd);
        assert(res, 'should support erc721');

        res = Asteroid::supports_interface(@state, 0x5b5e139f);
    ...
    fn test_set_sell_order() {
        ...
        let price = 1000_u128;
        Asteroid::mint_with_id(ref state, caller, token_id);
        Asteroid::set_sell_order(ref state, token_id, price);
        assert(Asteroid::get_sell_order(@state, token_id) == price, 'wrong price');

        Asteroid::set_sell_order(ref state, token_id, 0);
        assert(Asteroid::get_sell_order(@state, token_id) == 0, 'wrong price');
    }

    #[test]
    #[available_gas(10000000)]
    fn test_fill_sell_order() {
        let ast_class_hash: ClassHash = Asteroid::TEST_CLASS_HASH.try_into().unwrap();
        let mut constructor_data: Array<felt252> = Default::default();
        constructor_data.append('Influence Asteroids');
        constructor_data.append('INFAST');
        constructor_data.append('ADMIN');
        let (asteroid_address, _) = deploy_syscall(ast_class_hash, 0, constructor_data.span(), false).unwrap();
        let asteroid = IAsteroidDispatcher { contract_address: asteroid_address };
        ...
        asteroid.set_sway_address(sway_address);
        asteroid.add_grant(caller, Asteroid::roles::MINTER);

        let token_id = 42_u256;
        let price = 1000_u128;
        asteroid.mint_with_id(caller, token_id);
        asteroid.set_sell_order(token_id, price);
        ...
        asteroid.fill_sell_order(token_id);
        assert(asteroid.get_sell_order(token_id) == 0, 'wrong price');
        assert(asteroid.owner_of(token_id) == buyer, 'wrong owner');
    }
}

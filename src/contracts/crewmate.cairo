...
mod Crewmate {
    ...
    #[constructor]
    fn constructor(
        ref self: ContractState, name: felt252, symbol: felt252, initial_token: u128, admin: ContractAddress
    ) {
        self._name.write(name);
        self._symbol.write(symbol);
        self.token_tracker.write(initial_token.into());
    ...
    #[external(v0)]
    fn get_features(self: @ContractState, crewmate_id: u64) -> u128 {
        return self.features.read(crewmate_id);
    }

    #[external(v0)]
    fn set_features(ref self: ContractState, crewmate_id: u64, features: u128) {
        assert(self.role_grants.read((get_caller_address(), roles::ADMIN)), 'ERC721: must be admin');
        self.features.write(crewmate_id, features);
    }
    ...
    fn bridge_to_l1(ref self: ContractState, to_address: EthAddress, token_ids: Array<u128>) {
        assert(!to_address.is_zero(), 'invalid toAddress');
    ...
    // NOTE: include sender in calldata (based on change on L1)
    #[l1_handler]
    fn bridge_from_l1(
        ref self: ContractState, from_address: felt252, to_address: ContractAddress, token_ids: Array<felt252>
    ) {
        ...
        loop {
            ...
            let crewmate_id: u64 = (*token_ids[iter]).try_into().unwrap();
            let token_id_u256: u256 = crewmate_id.into();
            ...
            let features: u128 = (*token_ids[iter + 1]).try_into().unwrap();
            self.features.write(crewmate_id, features);
...
mod tests {
    ...
    use super::{Crewmate, ICrewmateDispatcher, ICrewmateDispatcherTrait};
    ...
    fn test_constructor() {
        ...
        let res = Crewmate::has_grant(@state, caller, Crewmate::roles::ADMIN);
    ...
    fn test_grants() {
        ...
        let res = Crewmate::has_grant(@state, caller, Crewmate::roles::MINTER);
        assert(res, 'caller should be minter');

        Crewmate::revoke_grant(ref state, caller, Crewmate::roles::MINTER);
        let res = Crewmate::has_grant(@state, caller, Crewmate::roles::MINTER);
    ...
    fn test_mint_without_grant() {
        ...
        Crewmate::mint_with_auto_id(ref state, caller);
    }
    ...
    fn test_transfer_wrong_sender() {
        ...
        let wrong_sender = starknet::contract_address_const::<'WRONG_SENDER'>();
        Crewmate::transfer_from(ref state, wrong_sender, receiver, token_id);
    }
    ...
    fn test_transfer_invalid_receiver() {
        ...
        Crewmate::transfer_from(ref state, caller, invalid_receiver, token_id);
    }
    ...
    fn test_approve_other_sender() {
        ...
        let token_id = Crewmate::mint_with_auto_id(ref state, caller);
        Crewmate::approve(ref state, other_sender, token_id);
        let res = Crewmate::getApproved(@state, token_id);
    ...
    fn test_send_with_approval() {
        ...
        Crewmate::transfer_from(ref state, caller, other_sender, token_id);
        let res = Crewmate::ownerOf(@state, token_id);
    ...
    fn test_token_uri() {
        ...
        Crewmate::set_base_uri(ref state, base_uri);

        let token_id = 42;
        Crewmate::mint_with_id(ref state, caller, token_id);
        let res = Crewmate::token_uri(@state, token_id);
        assert(*res.at(0) == 'https://api.influenceth.io/', 'wrong base uri');
        assert(*res.at(1) == 'metadata/crews/', 'wrong base uri');
        assert(*res.at(2) == '42', 'wrong base uri');
    }
    ...
    fn test_bridge_from_l1() {
        ...
        let mut state = Crewmate::contract_state_for_testing();
        Crewmate::constructor(ref state, 'Influence Crewmates', 'INFCRM', 20000, caller);
        Crewmate::add_grant(ref state, caller, Crewmate::roles::MINTER);

        Crewmate::set_l1_bridge_address(ref state, 'L1_BRIDGE'.try_into().unwrap());
        let token_ids = array![42, 82397293850685768012593140600065];
        Crewmate::bridge_from_l1(ref state, 'L1_BRIDGE', starknet::contract_address_const::<'PLAYER'>(), token_ids);

        let owner = Crewmate::ownerOf(@state, 42);
        let features = Crewmate::get_features(@state, 42);
        assert(owner == starknet::contract_address_const::<'PLAYER'>(), 'wrong owner');
        assert(features == 82397293850685768012593140600065, 'wrong features');
    }
    ...
    fn test_bridge_from_l1_fail() {
        ...
        Crewmate::set_l1_bridge_address(ref state, 'L1_BRIDGE'.try_into().unwrap());
        let token_ids = array![42, 82397293850685768012593140600065];
        Crewmate::bridge_from_l1(
            ref state, 'INCORRECT_L1_BRIDGE', starknet::contract_address_const::<'PLAYER'>(), token_ids
        );
    }
    ...
    fn test_erc165_response() {
        ...
        let mut res = Crewmate::supports_interface(@state, 0x01ffc9a7);
        assert(res, 'should support erc165');

        res = Crewmate::supports_interface(@state, 0x80ac58cd);
        assert(res, 'should support erc721');

        res = Crewmate::supports_interface(@state, 0x5b5e139f);
    ...
    fn test_set_sell_order() {
        ...
        let price = 1000_u128;
        Crewmate::mint_with_id(ref state, caller, token_id);
        Crewmate::set_sell_order(ref state, token_id, price);
        assert(Crewmate::get_sell_order(@state, token_id) == price, 'wrong price');

        Crewmate::set_sell_order(ref state, token_id, 0);
        assert(Crewmate::get_sell_order(@state, token_id) == 0, 'wrong price');
    }

    #[test]
    #[available_gas(10000000)]
    fn test_fill_sell_order() {
        let ast_class_hash: ClassHash = Crewmate::TEST_CLASS_HASH.try_into().unwrap();
        let mut constructor_data: Array<felt252> = Default::default();
        constructor_data.append('Influence Crewmates');
        constructor_data.append('INFCRM');
        constructor_data.append(20000);
        constructor_data.append('ADMIN');
        let (crewmate_address, _) = deploy_syscall(ast_class_hash, 0, constructor_data.span(), false).unwrap();
        let crewmate = ICrewmateDispatcher { contract_address: crewmate_address };
        ...
        crewmate.set_sway_address(sway_address);
        crewmate.add_grant(caller, Crewmate::roles::MINTER);

        let token_id = 42_u256;
        let price = 1000_u128;
        crewmate.mint_with_id(caller, token_id);
        crewmate.set_sell_order(token_id, price);
        ...
        crewmate.fill_sell_order(token_id);
        assert(crewmate.get_sell_order(token_id) == 0, 'wrong price');
        assert(crewmate.owner_of(token_id) == buyer, 'wrong owner');
    }
}

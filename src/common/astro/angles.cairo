...
fn D_to_M_near_parabolic(D: Fixed, ecc: Fixed) -> Fixed {
    ...
    assert(x.abs() < FixedTrait::ONE(), 'abs(x) must be less than 1');
    let S = _S_x(ecc, x);
...
fn M_to_E(M: Fixed, ecc: Fixed) -> Fixed {
    ...
    let E1 = loop {
        let f_val = E - ecc * E.sin() - M;
        let f_der = FixedTrait::ONE() - ecc * E.cos();
...
fn _S_x(ecc: Fixed, x: Fixed) -> Fixed {
    assert(x.abs() < FixedTrait::ONE(), 'abs(x) must be less than 1');
    let mut S = FixedTrait::ZERO();
    let mut k = FixedTrait::ZERO();
    let one = FixedTrait::ONE();
...
mod tests {
    ...
    fn test_M_to_E() {
        let mut E = super::M_to_E(FixedTrait::new_unscaled(1, false), FixedTrait::new(9223372036854775808, false));
        assert_precise(E, 27646155634686984192, 'invalid E1', Option::None(())); // 1.4987011
        E = super::M_to_E(FixedTrait::new_unscaled(3, false), FixedTrait::new(2305843009213693952, false));
        assert_precise(E, 55629764937411969024, 'invalid E2', Option::None(())); // 3.0156956
        E = super::M_to_E(FixedTrait::new_unscaled(4, false), FixedTrait::new(16602069666338596864, false));
        assert_precise(E, 66425956180200218624, 'invalid E3', Option::None(())); // 3.6009583
        E = super::M_to_E(FixedTrait::new_unscaled(5, false), FixedTrait::new(18262276632972455936, false));
        assert_precise(E, 76704957904068280320, 'invalid E4', Option::None(())); // 4.1581841
        E = super::M_to_E(FixedTrait::new_unscaled(4, true), FixedTrait::new(9223372036854775808, false));
    ...
    #[available_gas(100000000)]
    fn test_E_to_nu() {
        let mut nu = super::E_to_nu(FixedTrait::new_unscaled(4, true), FixedTrait::new(2305843009213693952, false));
        assert_precise(nu, 43799085820993306624, 'invalid nu', Option::None(())); // 2.3743532
        nu = super::E_to_nu(FixedTrait::new_unscaled(2, true), FixedTrait::new(9223372036854775808, false));
        assert_precise(nu, -44854733954750668800, 'invalid nu', Option::None(())); // -2.4315800
        nu = super::E_to_nu(FixedTrait::new_unscaled(0, false), FixedTrait::new(13835058055282163712, false));
        assert_precise(nu, 0, 'invalid nu', Option::None(())); // 0.0000000
        nu = super::E_to_nu(FixedTrait::new_unscaled(2, false), FixedTrait::new(16602069666338596864, false));
        assert_precise(nu, 52556323392500703232, 'invalid nu', Option::None(())); // 2.8490840
        nu = super::E_to_nu(FixedTrait::new_unscaled(4, false), FixedTrait::new(18262276632972455936, false));
    ...
    fn test_D_to_M_near_parabolic() {
        // 0.5, 0.995
        let mut M = super::D_to_M_near_parabolic(FixedTrait::new(9223372036854775808, false), FixedTrait::new(18354510353341003857, false));
        assert_precise(M, 10000066228410604000, 'invalid M1', Option::None(())); // 0.542104676492085
        // 0.5, 1.005
        M = super::D_to_M_near_parabolic(FixedTrait::new(9223372036854775808, false), FixedTrait::new(18538977794078099374, false));

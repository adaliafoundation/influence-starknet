...
mod RemovePrepaidPolicy {
    ...
    #[derive(Copy, Drop, starknet::Event)]
    enum Event {
        PrepaidPolicyRemoved: PrepaidPolicyRemoved
    }
    ...
    fn run(ref self: ContractState, target: Entity, permission: u64, caller_crew: Entity, context: Context) {
        ...
        assert(components::get::<PrepaidPolicy>(path).is_some(), 'no policy set');

        // Modify to "empty" and save
        components::set::<PrepaidPolicy>(path, PrepaidPolicyTrait::new(0, 0, 0));
        self.emit(PrepaidPolicyRemoved {
            entity: target,

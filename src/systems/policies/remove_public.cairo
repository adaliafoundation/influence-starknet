...
mod RemovePublicPolicy {
    ...
    #[derive(Copy, Drop, starknet::Event)]
    enum Event {
        PublicPolicyRemoved: PublicPolicyRemoved
    }
    ...
    fn run(ref self: ContractState, target: Entity, permission: u64, caller_crew: Entity, context: Context) {
        ...
        assert(components::get::<PublicPolicy>(path).is_some(), 'no policy set');

        // Modify to "empty" and save
        components::set::<PublicPolicy>(path, PublicPolicyTrait::new(false));
        self.emit(PublicPolicyRemoved {
            entity: target,
